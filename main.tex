...
This leads to the definition of spectral projectors and a discussion on the discrete and essential spectra in \Cref{sec:spectral-projectors}. 
...
\end{definition}
\footnotetext{
  Some, for example \cite{kreyszig1991introductory}, require only that the inverse is densely defined and bounded. 
...
\begin{enumerate}[label=(\alph*)]
  ...
  Since \Cref{thm:multiplication-operator-adjoint} gives the boundedness of \(M_{1 / (f - \lambda)}\), we have \(\lambda \in \res M_f\). 
...
\begin{proof}
  ...
  We assume \(\cH = L^2(X, \mu)\) and \(T = M_h\) with \(X\), \(\mu\), and \(h\) as in \Cref{thm:spectral-thm-self-adjoint}. 
...
Lastly, my sincere thanks to Alexandra Zhou, Hanlei Wen, Joshua Johnson, Ryan Dai, and Will Comess, for unwavering support and shared laughter. 
